...
cov_table<- new_data %>% summarise(across(c(4,6,7,11,12), function(x){list(n=sum(!is.na(x)), 
                                                                           missing=sum(is.na(x)),
...
kable(cov_table,
      caption= "Table 1- Summary of Covariates") %>% save_kable("Plots_Tables/Covariates_Summary.pdf")
...
kable(outcome_summary_table, col.names = c("Depression", "Number Obs", "Mean Days Drinking per Month", 
      ...
      caption= "Table 2- Summaries of Drinking for Each Depression Category") %>% save_kable("Plots_Tables/Table2-OutcomeSummaries.pdf")



#distributions of subject's answers to days_drank and days_heavy_drank
drinking_dist<- new_data %>% group_by(days_drank) %>% summarise(n=n())
heavy_drinking_dist<-new_data %>% group_by(days_heavy_drank) %>% summarise(n=n())

kable(new_data %>% group_by(days_drank) %>% summarise(n=n())) %>% save_kable("Plots_Tables/Days_DrankTable.pdf")
kable(new_data %>% group_by(days_heavy_drank) %>% summarise(n=n())) %>% save_kable("Plots_Tables/Days_Heavy_DrankTable.pdf")
...
ggplot(data=new_data, aes(x=days_drank)) +
  ...
  labs(title="Distribution of Days of Drinking",
       ...
       y="Count") +
  theme_bw()
ggsave("Plots_Tables/Distribution of Days Drinking.png", plot=last_plot())
ggplot(data=new_data, aes(x=days_heavy_drank)) +
  ...
  labs(title="Distribution of Days of Heavy Drinking",
       ...
       y="Count") +
  theme_bw()
ggsave("Plots_Tables/Distribution of Days Heavy Drinking.png", plot=last_plot())
...
ggsave("Plots_Tables/Distribution of Incomes.png", plot=last_plot())
ggplot(data=new_data, mapping=aes(log_income_2006)) +
  geom_histogram() +
  theme_bw() +
  labs(title="Distribution of Log-Incomes", x="Log(Income + 1)", y="Count")
ggsave("Plots_Tables/Distribution of Log-Incomes.png", plot=last_plot())